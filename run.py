...
class DockerWechatHook:
    ...
    def login_recovery_clicker(self):
        ...
        print("登录恢复点击监控已启动：首次扫码登录阶段不会点击")
    ...
    def click_login_button(self):
        ...
        if not window_id:
            print("未找到微信窗口，跳过本次登录恢复点击")
            return False

        geometry = self.get_window_geometry(window_id)
        if not geometry:
            print(f"无法读取微信窗口几何信息: {window_id}")
            return False
        ...
        if result.returncode == 0:
            print(f"已尝试点击登录按钮: window={window_id} x={click_x} y={click_y}")
            return True

        print(f"点击登录按钮失败: window={window_id} code={result.returncode}")
        return False
